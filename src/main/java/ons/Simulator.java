...
public class Simulator {
    ...
    public void Execute(String simConfigFile, String raClass, boolean trace, boolean verbose, boolean table, boolean json, double forcedLoad, int seed) {
        ...
        try {
            ...
            // 创建一个DocumentBuilderFactory实例，以供建立一个XML解析器。
            // 使用这个工厂类来创建一个DocumentBuilder实例。
            // 使用这个DocumentBuilder实例来解析一个具体的XML文件，并将解析后的内容作为一个Document对象返回，使得程序可以进一步以编程方式访问和操作XML文档中的数据
            DocumentBuilderFactory docBuilderFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder docBuilder = docBuilderFactory.newDocumentBuilder();
            Document doc = docBuilder.parse(new File(simConfigFile));

            // 标准化文档结构，并去除文档中的无用节点（比如空白节点）
            ...
            //检查XML文件版本与模拟器版本是否匹配，不匹配则打印错误信息并退出。
            ...
            /*
             * Extract simulation setup part 加载和解析模拟配置文件的典型步骤，这些设置决定了网络模拟的某些参数和行为方式。通过读取XML文件中的属性，程序能够以灵活的方式处理不同的模拟场景
             */
            begin = System.currentTimeMillis();
            if (Simulator.verbose) {
