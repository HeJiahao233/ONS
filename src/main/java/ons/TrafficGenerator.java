...
public class TrafficGenerator {
    ...
    public void generateTraffic(PhysicalTopology pt, EventScheduler events, int seed) {
        ...
        // Compute the weight vector 初始化权重向量 使用callsWeightVector和pairsWeightVector来根据不同类型的呼叫和流量对的权重分配随机事件。权重越大的类型在生成流量事件时被选中的概率越高。
        ...
        // Generate events 使用Distribution对象和随机数生成器来随机选择流量类型、源节点和目的节点，并计算事件发生的时间
